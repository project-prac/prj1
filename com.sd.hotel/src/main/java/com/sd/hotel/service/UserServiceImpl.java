...
public class UserServiceImpl implements UserService {
	...
	/*
	 * 구글 앱 비밀번호 설정 방법 1. 구글에 로그인한다. 2. [계정] - [보안] 3. [Google에 로그인하는 방법] - [2단계
	 * 인증]을 사용 설정한다. 4. 검색란에 "앱 비밀번호"를 검색한다. 5. 앱 이름을 "myapp"으로 작성하고 [만들기] 버튼을 클릭한다.
	...
	public ResponseEntity<Map<String, Object>> sendCode(Map<String, Object> params) {
		...
		myJavaMailUtils.sendMail((String) params.get("email"), "Hotel 회원가입 인증요청", "<div>인증코드는 " + code + " 입니다");
	...
	public void signup(HttpServletRequest request, HttpServletResponse response) {
		...
		MemberDto member = MemberDto.builder().userId(userId).email(email).password(password).name(name).tel(tel)
				.gender(gender).birth(birth).role(role).build();
	...
	public void modifyMember(HttpServletRequest request) {
		...
		if (role.equals("ROLE_USER")) {
			MemberDto member = MemberDto.builder().userId(userId).name(name).birth(birth).tel(tel).email(email).build();
			...
			// Authentication newauth = authenticationProvider.authenticate(new
			// UsernamePasswordAuthenticationToken(user,currentAuth.getCredentials()
			// ,user.getAuthorities() ));
			...
			EmployeeDto emp = EmployeeDto.builder().userId(userId).name(name).birth(birth).tel(tel).email(email).build();
	...
	public int modifyPw(HttpServletRequest request, HttpServletResponse response) {
		...
		if (role.equals("ROLE_USER")) {
			...
			/* 수정중 */
			modifyCount = memberMapper.updatePw(userId, newpw);
			...
			modifyCount = employeeMapper.updatePw(userId, newpw);
